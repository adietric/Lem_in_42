...
/*   Updated: 2019/11/14 19:24:43 by flafonso         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
					...
					unsigned long long len)
{
	...
	hach = hachage(gnl, len);
	room = inf_b->hach_table[hach];
	while (room != NULL)
	{
		if (ft_strncmp(gnl, room->name, (int)len) == 0)
			return (TRUE);
		room = room->next;
	}
	return (FALSE);
...
int					false_name_tube(char *gnl, t_info *inf_b, int i, int lennn)
{
	unsigned long long	len;
	int					len_tot;
	...
	while (len + lennn + 1 < len_tot)
	{
		...
		if (boucle_name(gnl, inf_b, len) == TRUE && len + 1 < len_tot
			&& boucle_name(&gnl[len + 1], inf_b, len_tot - len - 1) == TRUE)
		{
			inf_b->len[0] = len;
